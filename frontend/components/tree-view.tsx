...
function TreeNodeItem({ node, depth }: { node: TreeNode; depth: number }) {
  ...
  return (
    <Collapsible defaultOpen={depth < 2}>
      <CollapsibleTrigger
        className="flex w-full items-center gap-2 rounded py-1 text-sm text-zinc-300 hover:bg-zinc-800/50"
        style={{ paddingLeft: `${depth * 16}px` }}
      >
        <ChevronRight className="h-4 w-4 shrink-0 text-zinc-500 transition-transform [[data-panel-open]_&]:rotate-90" />
        {nodeId && (
          <Badge variant="secondary" className="text-[10px] font-mono">
            {nodeId}
          </Badge>
...
export function TreeView({ tree_json }: TreeViewProps) {
  ...
  if (root.title || root.children) {
    return (
      <div className="max-h-80 overflow-y-auto rounded-md bg-zinc-950 p-2">
